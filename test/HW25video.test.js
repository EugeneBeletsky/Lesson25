...
const googleJsonSchemaVideo = require('../testData/googleSchemaVideo.json');

describe('google API tests', () => {

    let response;
    test('first test', async () => {
        response = await axios.get('https://google-search3.p.rapidapi.com/api/v1/video/q=iphone+reviews', {
            headers: {
    ...
    test ('validate json schema', async () => {
        console.log('googleJsonSchemaImage', googleJsonSchemaVideo);
                await expect(response.data).toBeValidSchema(googleJsonSchemaVideo);
    })
    ...
    test ('json schema validation', async() => {
        ...
        await expect(response).toBeValidSchema(googleJsonSchemaVideo);      
    })
